...
~/bin/sequoia sdk -command "-I spock-basic.ini --install-skip true --rebound 90 --bucket-password="password" --testsuite-test=Rb1Swap --testsuite-variants=HYBRID -d all:debug -A S3Creds_tmp -C share/rexec --rexec_path=/root/sdkd-java/run-sdkd-java --rexec_port=8050" -container_name ${CONTAINER_NAME}
